...
int main(int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "0")) {
        ...
        if (!strcmp(argv[1], "test")){ //Unit test
            runUnitTest(init_n, max_n, max_t, verbose, block_size, sleep_time, split_chance, true);
        }
        ...
        //MVP
            ...
            runMVP(init_n, max_n, max_t, 0, verbose, block_size, sleep_time, 0.1, split_chance, true);
        }
        else if (!strcmp(argv[1], "21")){ // MVP CPU Sync
            runMVP(init_n, max_n, max_t, 1, verbose, block_size, sleep_time, 0.1, split_chance, true);
        }
        else if (!strcmp(argv[1], "22")){ // MVP Static
            runMVP(init_n, max_n, max_t, 2, verbose, block_size, sleep_time, 0.1, split_chance, true);
        }
        else if (!strcmp(argv[1], "23")){ // MVP Dynamic
            runMVP(init_n, max_n, max_t, 3, verbose, block_size, sleep_time, 0.1, split_chance, true);
        }
    }
