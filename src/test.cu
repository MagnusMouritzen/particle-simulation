...
void runBenchmark(){
                    ...
                    for(int max_n : max_ns){
                        for(int sleep_time : sleep_times){
                            for(int function : functions){
                                if (function == 2 && block_size > 256) continue;
                                RunData run_data = runPIC(init_n, max_n, poisson_steps, mobility_steps, function, 0, block_size, sleep_time);
    ...
    printCSV(data, "out/data/mobility_timesteps_nodet.csv");
}
